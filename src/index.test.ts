...
async function getClient() {
  if (client)
    return client
  const { RPC_URL, RPC_SECRET } = process.env
  if (!RPC_URL)
    throw new Error('RPC_URL is undefined')
  const url = new URL(RPC_URL || 'http://localhost:10200')

  client = new NimiqRPCClient(url, { secret: RPC_SECRET || '' })
...
describe('test for blockchain module', async () => {
  ...
  it('.get blockNumber w/txs ok', async () => expect((await blockchain.getBlockByNumber(blockNumber, { includeTransactions: true })).data).toHaveProperty('hash'))
  it('.latest ok', async () => expect((await blockchain.getLatestBlock()).data).toHaveProperty('hash'))
  it('.latest ok w/txs', async () => expect((await blockchain.getLatestBlock({ includeTransactions: true })).data).toHaveProperty('transactions'))
